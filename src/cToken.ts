...
/**
 * Supplies the user's Ethereum asset to the Justpound protocol.
 *
...
export async function supply(asset: string, amount: any, options: any = {}) {
  await netId(this);
  const errorPrefix = 'Justpound [supply] | ';
  ...
  if (cTokenName !== constants.cETH) {
    ...
    await eth.trx(
      ...
      { _JustpoundProvider: this._provider, abi: abi.cErc20 }
    );
  }

  const trxOptions: any = { _JustpoundProvider: this._provider };
  const parameters = [];
  if (cTokenName === constants.cETH) {
...
/**
 * Redeems the user's Ethereum asset from the Justpound protocol.
 *
 ...
 *   const trx = await Justpound.redeem(Justpound.ETH, 1); // also accepts cToken args
 *   console.log('Ethers.js transaction object', trx);
 * 
 * })().catch(console.error);
 * ```
...
export async function redeem(asset: string, amount: any, options: any = {}) {
  await netId(this);
  const errorPrefix = 'Justpound [redeem] | ';
...
/**
 * Borrows an Ethereum asset from the Justpound protocol for the user. The user's
 ...
 *   const trx = await Justpound.borrow(Justpound.DAI, daiScaledUp, trxOptions);
 * 
 *   console.log('Ethers.js transaction object', trx);
 * 
 * })().catch(console.error);
...
export async function borrow(asset: string, amount: any, options: any = {}) {
  await netId(this);
  const errorPrefix = 'Justpound [borrow] | ';
  ...
  const trxOptions: any = { _JustpoundProvider: this._provider, ...options };
  const parameters = [ amount ];
 ...
 *   const trx = await Justpound.repayBorrow(Justpound.DAI, 32, address);
 * 
 *   console.log('Ethers.js transaction object', trx);
 * 
 * })().catch(console.error);
...
export async function repayBorrow(asset: string, amount: any, borrower: string, options: any = {}) {
  await netId(this);
  const errorPrefix = 'Justpound [repayBorrow] | ';
