...
/**
 * Enters the user's address into Justpound protocol markets.
 *
 ...
 * 
 * (async function () {
 *   const trx = await Justpound.enterMarkets(Justpound.ETH); // Use [] for multiple
...
export async function enterMarkets(markets: any = []) {
  await netId(this);
  const errorPrefix = 'Justpound [enterMarkets] | ';
...
/**
 * Exits the user's address from a Justpound protocol market.
 *
...
export async function exitMarket(market: string) {
  await netId(this);
  const errorPrefix = 'Justpound [exitMarkets] | ';
