 ...
 *     https://Justpound.finance/docs/api#AccountService
 *
 ...
 *     API documentation. https://Justpound.finance/docs/api#CTokenService
 *
 * @param {object} options A JavaScript object of API request parameters.
 *
 * @returns {object} Returns the HTTP response body or error.
 ...
 * ```
 * (async function() {
 *   const cDaiData = await Justpound.api.cToken({
 *     "addresses": Justpound.util.getAddress(Justpound.cDAI)
 *   });
 * 
 ...
 *     documentation. https://Justpound.finance/docs/api#MarketHistoryService
 *
 * @param {object} options A JavaScript object of API request parameters.
 *
 * @returns {object} Returns the HTTP response body or error.
 ...
 * ```
 * (async function() {
 *   const cUsdcMarketData = await Justpound.api.marketHistory({
 *     "asset": Justpound.util.getAddress(Justpound.cUSDC),
 ...
 *     https://Justpound.finance/docs/api#GovernanceService
 *
 * @param {object} options A JavaScript object of API request parameters.
 ...
 * ```
 * (async function() {
 *   const proposal = await Justpound.api.governance(
...
function queryApi(options: object, name: string, path: string) {
  return new Promise(async (resolve, reject) => {
    const errorPrefix = `Justpound [api] [${name}] | `;
    let responseCode, responseMessage;

    try {
      const response = await request({
        hostname: 'https://api.Justpound.finance',
        path,
        method: 'POST',
        headers: {
          'Content-type': 'application/json',
          // 'Justpound-js': `[${version}]_[${userPlatform}]`,
        },
        body: options
      });
      ...
      if (responseCode >= 200 && responseCode <= 299) {
        ...
        throw 'Invalid request made to the Justpound API.';
      }
    } catch(error) {
      let errorMessage = '';
